-- In-game dev panel ("DEVPAN" button above the menu logo, js/menu/devpan.js):
-- a quick stats summary + who is connected right now (accounts with their
-- cell/frame, and guests), with live spectating of solo games.
-- Only accounts listed in dev_admins can call it (the button is only drawn
-- for window.DEV_UID, but the data is gated HERE, server-side).

create table if not exists public.dev_admins (
  user_id uuid primary key references auth.users(id) on delete cascade
);
alter table public.dev_admins enable row level security; -- no policies: functions only
insert into public.dev_admins (user_id) values ('530cb816-8562-4e7e-a538-15c6713fcc8d') -- BlueLite
  on conflict do nothing;

create or replace function public.dev_panel_summary()
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  select jsonb_build_object(
    ...
    'last24h', jsonb_build_object(
      'visitors',     (select count(distinct visitor_id) from public.analytics_events where type = 'visit' and created_at >= v_day),
      'games',        (select count(*) from public.analytics_events where type in ('campaign', 'versus', 'globequiz') and created_at >= v_day),
      'mode_games',   (select count(*) from public.analytics_events where type = 'game' and coalesce(session_type, '') <> 'practice' and created_at >= v_day),
      'new_accounts', (select count(*) from public.profiles where created_at >= v_day),
      'active_accounts', (select count(*) from public.profiles where last_active >= v_day),
...
revoke all on function public.dev_panel_summary() from public, anon;
grant execute on function public.dev_panel_summary() to authenticated;
